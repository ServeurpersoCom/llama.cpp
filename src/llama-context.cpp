...
// maps a tensor to the context backend that owns its buffer, so deferred reads can
// use the async backend interface instead of one blocking transfer per tensor
static ggml_backend_t llama_io_backend_for(const std::vector<ggml_backend_ptr> * backends, const ggml_tensor * t) {
    if (backends == nullptr || t->buffer == nullptr) {
        return nullptr;
    }
...
public:
    llama_io_write_host(
            uint8_t * p, size_t len, const std::vector<ggml_backend_ptr> * backends = nullptr) : ptr(p), buf_size(len), backends(backends) {}

    ~llama_io_write_host() {
        // queue the reads on their owning backends and synchronize once, so backends
        // with an async implementation overlap the transfers instead of paying one
...
size_t llama_state_seq_set_data_range(llama_context * ctx, const uint8_t * src, size_t size, llama_seq_id dest_seq_id, llama_state_seq_flags flags, llama_pos p0, llama_pos p1) {
    GGML_ASSERT(dest_seq_id >= 0);

    ctx->synchronize();

    return ctx->state_seq_set_data(dest_seq_id, src, size, flags, p0, p1);
}
