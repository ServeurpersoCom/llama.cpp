...
void llama_model_qwen4exp::load_arch_tensors(llama_model_loader & ml) {
    ...
    // an MTP only file carries the draft block alone, so the trunk tensors are absent
    ...
    // the MTP draft block sits one past the trunk. it is a full qwen4exp layer that attends
    // dense, plus the three nextn tensors, and is read only when the file is opened as a draft
...
llama_model_qwen4exp::graph::graph(const llama_model & model, const llm_graph_params & params) :
    ...
    // the MTP head reads the wide residual, before the final mix collapses it. res_hc is
    // passed as is rather than reshaped: a bare view is not a node the scheduler places, and
    // the extraction is a flat copy, for which [n_embd, hc, T] and [hc*n_embd, T] are identical
    res->t_h_nextn = res_hc;

    // an unmasked extraction wants every row, so the gather waits until the head has its state
...
ggml_tensor * llama_model_qwen4exp::graph::build_layer_attn(
    ...
    // the MTP draft block runs without a hybrid cache, so it is always dense
