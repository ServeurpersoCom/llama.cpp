 ...
 *
 * The store confirms to `ApiFilesystemSearchEntry` so the picker can
 * render it with no shape conversion. `size` is only present for files;
 * JSON round-trip preserves that and the picker doesn't depend on it.
 *
 * Persistence notes:
 * - strict-shape validation on read: any item missing `path`, `name`,
 *   `type`, or `modified` is dropped rather than crashing the picker.
...
function isValidEntry(value: unknown): value is ApiFilesystemSearchEntry {
	if (!value || typeof value !== 'object') return false;
	const entry = value as Partial<ApiFilesystemSearchEntry>;
	return (
...
class RecentMentionsStore {
	items = $state<ApiFilesystemSearchEntry[]>(this.loadFromStorage());

	get value(): ApiFilesystemSearchEntry[] {
		return this.items;
	}
	 ...
	 */
	add(entry: ApiFilesystemSearchEntry) {
	...
	private persist(next: ApiFilesystemSearchEntry[]) {
		if (!browser) return;
		try {
	...
	private loadFromStorage(): ApiFilesystemSearchEntry[] {
		if (!browser) return [];
		try {
