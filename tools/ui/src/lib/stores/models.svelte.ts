...
/**
 * Pending awaiter for a model status transition reported by the feed.
 */
interface ModelStatusWaiter {
...
class ModelsStore {
	...
	private statusWaiters = new Map<string, ModelStatusWaiter[]>();
		...
		while (!signal.aborted) {
			try {
				...
				if (response.ok && response.body) {
					await this.consumeStatusStream(response.body, signal);
				}
			} catch {
	...
	/**
	 * Split the byte stream into SSE records on the blank-line boundary.
	 */
	private async consumeStatusStream(
		...
		while (!signal.aborted) {
			...
			while (boundary !== -1) {
				...
				this.handleStatusRecord(record);
	...
	private applyModelStatus(event: ApiModelsSseEvent): void {
		...
		if (status === ServerModelStatus.LOADING) {
			this.trackLoadProgress(model, data.progress);
		} else {
		...
		if (failed) {
			this.rejectStatusWaiters(model, new Error(`Model failed: ${this.toDisplayName(model)}`));
			return;
		}

		this.resolveStatusWaiters(model, status);
	}
	...
	private removeRouterModel(modelId: string): void {
		...
		this.rejectStatusWaiters(modelId, new Error(`Model removed: ${this.toDisplayName(modelId)}`));
	}
	...
	/**
	 * Track progress, keeping the last stage across updates so bare loading
	 * events keep their label.
	 */
	private trackLoadProgress(modelId: string, progress?: ApiModelsSseProgress): void {
		const existing = this.loadProgress.get(modelId);

		this.loadProgress.set(modelId, {
	...
	private waitForStatus(modelId: string, target: ServerModelStatus): Promise<void> {
		return new Promise<void>((resolve, reject) => {
			const waiters = this.statusWaiters.get(modelId) ?? [];
			waiters.push({ target, resolve, reject });
			this.statusWaiters.set(modelId, waiters);
		});
	}

	/**
	 * Resolve and drop awaiters whose target matches the reported status.
	 */
	private resolveStatusWaiters(modelId: string, status: ServerModelStatus): void {
		const waiters = this.statusWaiters.get(modelId);
		if (!waiters) return;

		const remaining = waiters.filter((waiter) => {
			if (waiter.target === status) {
				waiter.resolve();
				return false;
			}
			return true;
		...
		if (remaining.length > 0) {
			this.statusWaiters.set(modelId, remaining);
		} else {
	...
	/**
	 * Reject and drop all awaiters for a model.
	 */
	private rejectStatusWaiters(modelId: string, error: Error): void {
		const waiters = this.statusWaiters.get(modelId);
		if (!waiters) return;
	...
	/**
	 * Reject and drop every pending awaiter, used on teardown when the feed
	 * can no longer report completion.
	 */
	private rejectAllStatusWaiters(error: Error): void {
	...
	async loadModel(modelId: string): Promise<void> {
		...
		try {
			...
			this.rejectStatusWaiters(modelId, error instanceof Error ? error : new Error('load failed'));
	...
	async unloadModel(modelId: string): Promise<void> {
		...
		try {
			...
			this.rejectStatusWaiters(
				modelId,
				error instanceof Error ? error : new Error('unload failed')
			);
	...
	clear(): void {
		...
		this.rejectAllStatusWaiters(new Error('Models store cleared'));
		this.models = [];
		this.routerModels = [];
		this.loading = false;
		this.updating = false;
