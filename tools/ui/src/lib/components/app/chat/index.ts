 ...
 * Add dropdown. The X on the chip is a two-step dismiss: first click clears
 * an existing directory, second click fires `onDismiss` so the parent can
 * hide it entirely. The trigger opens a popover with the picker UI (text
 * search against `/v1/filesystem/search`, browse via File System Access
 * API); the picked directory is exposed via `bind:directory`. When the cwd
 * sits inside a git repo, the trigger shows an inline branch badge - probed
 * via `/v1/filesystem/git`.
 */
