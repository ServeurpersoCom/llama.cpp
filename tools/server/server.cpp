...
int llama_server(common_params & params, int argc, char ** argv) {
    ...
    if (is_router_server) {
        ...
        ctx_http.post("/models/config",        ex_wrapper(models_routes->post_router_model_config));
        ctx_http.get ("/models/max-context",   ex_wrapper(models_routes->get_router_model_max_context));
