...
// Load tools definitions from sandbox-tools.json
const TOOLS_DEFINITIONS = JSON.parse(
	fs.readFileSync(path.join(__dirname, 'sandbox-tools.json'), 'utf8')
).map((tool) => ({
...
async function tool_view(args) {
	...
	if (isDir) {
		// Directory listing (non-recursive, like Claude)
		...
		if (range && Array.isArray(range) && range.length === 2) {
			startLine = Math.max(1, parseInt(range[0]) || 1);
			endLine =
				range[1] === -1
					? totalLines
					: Math.min(totalLines, parseInt(range[1]) || totalLines);
		}
 ...
 * @param {object} args - Tool arguments
 * @param {string} args.path - File path to create
 * @param {string} args.file_text - File content
 * @returns {Promise<string>} Tool result
 */
...
/**
 * Tools mapping for dynamic dispatch
